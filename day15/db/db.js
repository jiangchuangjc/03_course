...
exports.find = function(collection,filter,option,callback){
   ...
   if(arguments.length==2){
     //第一个表示集合,第二个表示回调
     //将回调函数重新赋值给callback,并将filter设置成默认值
     callback = filter;
     filter = {};
     //设置option默认值{start:1}
  ...
  //  console.log(callback);
   ...
   //数据库查询时,跳过的是具体的条数
   //获取跳过的条数
   var skip = (option.start-1)*option.size; //获取跳过的条数。
   var size = option.size //获取每页显示的条数
   var order = option.order; //获取排序的依据
